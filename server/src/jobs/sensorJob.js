...
const initSensorJob = () => {
    node_cron_1.default.schedule('*/10 * * * * *', () => {
        ...
        for (const facility of facilities) {
            ...
            const stalls = setup_1.db.prepare('SELECT * FROM stall_status WHERE facility_id = ?').all(facility.id);
            for (const stall of stalls) {
                if (Math.random() > 0.8) {
                    const newStatus = stall.is_occupied === 1 ? 0 : 1;
                    setup_1.db.prepare('UPDATE stall_status SET is_occupied = ?, last_updated = ? WHERE id = ?')
      ...
      `).run(facility.id, ammonia, humidity, floor_wet, 1, tissue, soap, new Date().toISOString());
            ...
            const lastStatus = setup_1.db.prepare('SELECT status FROM cleanliness_status WHERE facility_id = ? ORDER BY id DESC LIMIT 1').get(facility.id);
            if (!lastStatus || lastStatus.status !== status) {
                setup_1.db.prepare('INSERT INTO cleanliness_status (facility_id, status, reason, updated_at) VALUES (?, ?, ?, ?)')
                    .run(facility.id, status, reason, new Date().toISOString());
                index_1.io.emit('status_change', {
            ...
            setup_1.db.prepare('INSERT INTO crowd_queue (facility_id, current_users, wait_time_mins, pressure_level, timestamp) VALUES (?, ?, ?, ?, ?)')
                .run(facility.id, currentUsers, waitTime, pressure, new Date().toISOString());
            // 5. Emit Events
            index_1.io.emit('sensor_update', {
                facility_id: facility.id,
                ammonia,
                humidity,
                floor_wet,
            ...
            if (status === 'RED' || (tissue < 20) || (soap < 15)) {
                index_1.io.emit('maintenance_alert', {
                    facility_id: facility.id,
...
exports.initSensorJob = initSensorJob;
//# sourceMappingURL=sensorJob.js.map